 ...
 * It uses the UserService, UserRepository, CartService and CartRepository
 * classes
 * to perform the business logic and data access operations.
 * It also uses the SecurityContextHolder class to get the authenticated user
 * name.
 ...
 * 
 * @param userDTO a UserRequest object that contains the updated user
 *                information.
 * @return a ResponseEntity object with a MiscResponse object as the body and OK
 *         status code.
 * @throws UserNotFoundException if the user does not exist in the database.
 * @throws IOException           if there is an error in reading or writing the
 *                               user image file.
 */
public class UserController {
        ...
        public ResponseEntity<MiscResponse> giveReview(@RequestBody ReviewRequest review) {
                ...
                return ResponseEntity.ok(MiscResponse.builder()
                                .response(productReviewService.giveReviewRating(user, review)).build());
