 ...
 * It provides functionality to allow users to give reviews and ratings for
 * products.
 ...
 * productRepository: an instance of ProductRepository used to fetch product
 * information.
 * orderRepository: an instance of OrderRepository used to fetch order
 * information.
 * The giveReviewRating method accepts a User object and a ReviewRequest object
 * as parameters
 * and returns a string indicating the result of the review submission. It first
 * retrieves the
 * user's orders and checks if the requested product is present in any of the
 * orders. If so,
 * it retrieves the product from the productRepository and creates a
 * ReviewRating object using
 * the user's information and the review request. The product's rating is then
 * updated by taking
 * into account the existing ratings and the new rating provided. The review is
 * added to the
 * product's reviewRating map. Finally, the updated product is saved using the
 * productRepository.
 * If the requested product is not found in any of the user's orders, the method
 * returns
 * "You Cant Review" indicating that the user is not eligible to review the
 * product.
 * If the review submission is successful, the method returns "Thank You for
 * your Review".
 */
        ...
        for (Order order : orders) {
            if (order.getOrderItems().stream().anyMatch(item -> item.getProductId().equals(request.getProductId()))) {
                ...
                ReviewRating reviewRating = ReviewRating.builder()
                        ...
                        .build();
                product.setRating((product.getRating() * product.getReviewRating().size() + request.getRating())
                        / (product.getReviewRating().size() + 1));
